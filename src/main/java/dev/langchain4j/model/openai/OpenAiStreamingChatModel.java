...
public class OpenAiStreamingChatModel implements StreamingChatModel {
  ...
  public OpenAiStreamingChatModel(OpenAiStreamingChatModelBuilder builder) {
    ...
    if (builder.defaultRequestParameters
        instanceof OpenAiChatRequestParameters openAiChatRequestParameters) {
    ...
    this.defaultRequestParameters =
        OpenAiChatRequestParameters.builder()
            ...
            .frequencyPenalty(
                getOrDefault(builder.frequencyPenalty, commonParameters.frequencyPenalty()))
            .presencePenalty(
                getOrDefault(builder.presencePenalty, commonParameters.presencePenalty()))
            ...
            .responseFormat(
                getOrDefault(
                    ...
                    commonParameters.responseFormat()))
            // OpenAI-specific parameters
            .maxCompletionTokens(
                getOrDefault(builder.maxCompletionTokens, openAiParameters.maxCompletionTokens()))
            .logitBias(getOrDefault(builder.logitBias, openAiParameters.logitBias()))
            .parallelToolCalls(
                getOrDefault(builder.parallelToolCalls, openAiParameters.parallelToolCalls()))
      ...
      for (ToolCall toolCall : toolCalls) {
        ...
        if (isNotNullOrEmpty(partialArguments)) {
          ...
          ToolExecutionRequest partialToolExecutionRequest =
              ToolExecutionRequest.builder().id(id).name(name).arguments(partialArguments).build();
          try {
  ...
  public static class OpenAiStreamingChatModelBuilder {
    ...
    /**
     * Sets default common {@link ChatRequestParameters} or OpenAI-specific {@link
