...
/**
 * Handles response from a language model for AI Service that is streamed token-by-token. Handles
 * both regular (text) responses and responses with the request to execute one or multiple tools.
 */
@Internal
class AiServiceStreamingResponseHandler implements StreamingChatResponseHandler {
  ...
  public void onPartialResponse(String partialResponse) {
    // If we're using output guardrails, then buffer the partial response until the guardrails have
    // completed
  ...
  @Override
  public void onPartialToolExecutionRequest(
      int index, ToolExecutionRequest partialToolExecutionRequest) {
    // If we're using output guardrails, then buffer the partial response until the guardrails have
    // completed
      ...
      if (completeResponseHandler != null) {
        ChatResponse finalChatResponse =
            ChatResponse.builder()
                .aiMessage(aiMessage)
                .metadata(
                    completeResponse.metadata().toBuilder()
        ...
        if (hasOutputGuardrails) {
          ...
          // If we have output guardrails, we should process all of the partial responses first
          // before
          // completing
