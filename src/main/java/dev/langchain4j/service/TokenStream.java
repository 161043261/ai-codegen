...
/**
 * Represents a token stream from the model to which you can subscribe and receive updates when a
 * new partial response (usually a single token) is available, when the model finishes streaming, or
 * when an error occurs during streaming. It is intended to be used as a return type in AI Service.
 */
public interface TokenStream {

  /**
   * The provided consumer will be invoked every time a new partial response (usually a single
   * token) from a language model is available.
   *
   * @param partialResponseHandler lambda that will be invoked when a model generates a new partial
   *     response
   * @return token stream instance used to configure or start stream processing
   */
  TokenStream onPartialResponse(Consumer<String> partialResponseHandler);

  TokenStream onPartialToolExecutionRequest(
      BiConsumer<Integer, ToolExecutionRequest> toolExecutionRequestHandler);

  TokenStream onCompleteToolExecutionRequest(
      BiConsumer<Integer, ToolExecutionRequest> completedHandler);

  /**
   * The provided consumer will be invoked if any {@link Content}s are retrieved using {@link
   * RetrievalAugmentor}.
   *
   * <p>The invocation happens before any call is made to the language model.
   *
   ...
   *
   * <p>The invocation happens after the tool method has finished and before any other tool is
   * executed.
   *
   ...
   *
   * @param completeResponseHandler lambda that will be invoked when language model finishes
   *     streaming
   * @return token stream instance used to configure or start stream processing
   */
