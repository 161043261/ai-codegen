...
public class MermaidDiagramTool {
  ...
  public List<ImageResource> generateMermaidDiagram(
      @P("Mermaid 图表代码") String mermaidCode, @P("架构图描述") String description) {
    ...
    try {
      ...
      // 保存到本地存储
      String keyName =
          String.format("/mermaid/%s/%s", RandomUtil.randomString(5), diagramFile.getName());
      String fileUrl = localStorageManager.uploadFile(keyName, diagramFile);
      // 清理临时文件
