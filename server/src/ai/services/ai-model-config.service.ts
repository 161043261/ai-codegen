  ...
  createStreamingChatModel(): BaseChatModel {
    if (this.isOllama) {
      ...
      return new ChatOllama({
        ...
        model: this.configService.get('OLLAMA_STREAMING_CHAT_MODEL', 'qwen3'),
    ...
    return new ChatOpenAI({
      ...
      maxTokens: this.configService.get<number>(
        'STREAMING_CHAT_MODEL_MAX_TOKENS',
        8192,
      ),
      streaming: true,
    });
  ...
  createRouteChatModel(): BaseChatModel {
    if (this.isOllama) {
      ...
      return new ChatOllama({
        ...
        model: this.configService.get('OLLAMA_ROUTE_CHAT_MODEL', 'qwen3'),
        numPredict: this.configService.get<number>(
          'OLLAMA_ROUTE_CHAT_MAX_TOKENS',
          100,
        ),
      });
    }
